...
def load_full_data(data_path='./data/shuffleSlots_extend.xlsx'):
    full_df = pd.read_excel(data_path, sheet_name='shuffleData')
    full_df = full_df[~full_df['CNTR_N'].isna()]
    return full_df


def load_blk_data(blk_no, data_path='./data/shuffleSlots_extend.xlsx'):
...
def load_config(data_path='./config/config_smartShuffling.xlsx'):
    ...
    shuffle_requirements = np.array(excel_config.columns)
    terminal_shuffle_requirements = {}

    for terminal in excel_config.index:
        terminal_config = np.array(excel_config.loc[terminal])
        terminal_shuffle_requirements[terminal] = shuffle_requirements[np.where(
            terminal_config == 1)[0]]

    return terminal_shuffle_requirements
...
def preprocess(slot_df, box_freeze=False):
    if box_freeze:
        columns = ['row', 'level', 'CNTR_N', 'vsl2', 'pscw',
                   'SNAP_DT', 'etb2', 'etu2', 'pk', 'sz',	'cat',	'wtClass', 'boxFreeze']
    else:
        columns = ['row', 'level', 'CNTR_N', 'vsl2', 'pscw',
                   'SNAP_DT', 'etb2', 'etu2', 'pk', 'sz',	'cat',	'wtClass']
